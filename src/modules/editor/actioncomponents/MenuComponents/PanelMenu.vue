<template>
  <div class="p-panelmenu p-component" @drop="onDrop($event, 1)">
        ...
        <div :class="getHeaderClass(item)" :style="item.style">
          <template v-if="!$slots.item">
            <router-link v-if="item.to && !disabled(item)" :to="item.to" custom
              ...
              <a :href="href" :class="getHeaderLinkClass(item, {isActive, isExactActive})"
            ...
            <a v-else :href="item.url" :class="getHeaderLinkClass(item)" @click="onItemClick($event, item)"
               draggable="true" @dragstart="startDrag($event, item)"
               @dragover.prevent @dragenter.prevent :tabindex="disabled(item) ? null : '0'"
...
export default {
  ...
  methods: {
    ...
    onDrop(evt, list) {
      console.log("This is called in onDrop", evt);
